...
- **6개 번역 동시 표시**: 개역개정 · 새번역 · 우리말성경 · 현대인의 성경 · NIV · ESV (체크박스로 켜고 끄기)
- **여러 구절을 한 화면에** — 떨어진 구절들을 *패널*에 담아 좌→우로 최대 4개까지 나란히 비교. 상단 입력란에 자유 텍스트로 한 번에 입력 (`Acts 12:12, 12:25, 13:5, 15:37-40, Col 4:10, Philem 1:24`) — 책 약어·접두사(`Col`→Colossians), 책 생략 시 이어쓰기(`12:25`→앞 구절의 책), `:`/`.` 구분, `-`/`~`/`–` 범위, `||` 로 패널 나누기를 모두 인식
- **장 전체 · 장 범위** — 절 옆 *전체* 체크로 한 장을 통째로, `Acts 13-15` 처럼 **여러 장**도 한 번에
- **한글 단위 입력** — `롬 11장`, `요 3장 16절`, `롬 11장-13장` 같은 한글 "장/절" 표기도 그대로 인식
- **패널 편집** — 각 구절 블록에 ◀▶(옆 패널로 이동) · ↦(특정 패널 번호로 이동) · ↑↓(패널 안 순서) · ✂(절 범위를 위/아래로 분할) · 📋(본문 복사) · ★(북마크) · ✕(제거), 패널 상단에 🗑(패널 삭제). *＋추가* 는 **선택된 패널**에 쌓고, *＋새 패널* 로 빈 칸을 만들어 ◀▶로 옮겨 담기. **재정렬·이동·삭제·분할은 재조회 없이 즉시**
- **패널 선택 + 패널별 역본** — 패널을 클릭하면 선택(파란 테두리)되고, 위쪽 역본 체크박스·순서가 **그 패널에만** 적용됩니다. 패널마다 다른 번역 조합·순서로 볼 수 있어요. *＋추가* 도 선택된 패널로 들어갑니다
- **역본 순서 조정** — 필터 줄의 *역본 순서* 로 번역본 표시 순서를 위/아래로 변경 (패널별로 따로 지정 가능)
- **번갈아보기** — 패널마다 켜면 *절 단위*로 여러 번역을 묶어 표시 (16 → 개역 · 현대인 · NIV …). 꺼두면 번역본 단위
- **본문 복사 · 북마크** — 절 블록의 📋 로 헤더+본문을 클립보드에 복사, ★ 로 즐겨찾기에 저장. *북마크* 메뉴에서 목록 보기·불러오기·삭제
- **되돌리기** (↶ / Ctrl+Z) — *새로 보기* 로 화면을 덮어쓰거나 편집한 직후 직전 상태로 복원 (최대 30단계)
- **글자 크기 조절** — 필터 줄의 `A−` / `A+`, 또는 `pt` 숫자를 직접 입력. Ctrl+= / Ctrl+- / Ctrl+0(기본값)
- **미리보기 팝업** — 입력란에서 **Enter** 를 누르면 화면을 건드리지 않고 팝업으로 미리보기. 팝업에서 *새로 보기* / *＋추가* 로 화면에 반영
...
- **라이브러리** — *라이브러리* 메뉴: 현재 패널 구성(위치·묶음·번갈아보기·패널별 역본 포함)을 이름 붙여 저장하고 언제든 불러오기(교체)/추가/삭제
...
- **키워드 검색** (Ctrl+F) — 캐시된 본문에서 한/영 부분 일치 검색. 결과 더블클릭하면 **선택된 패널에 추가** (다이얼로그는 열린 채라 계속 추가 가능)
...
- **마우스 호버 설명** — 주요 버튼·입력칸에 커서를 올리면 툴팁으로 무엇을 하는지 설명
...
1. 상단에서 **책**·**장**·**절 범위**(또는 *전체*)를 고르고 **새로 보기** (Ctrl+Enter). 화면이 그 구절 한 패널로 바뀝니다. (*새로 보기* = 기존 화면을 지우고 새로 채움)
   - 또는 그 아래 입력란에 여러 구절을 한 번에: `Acts 12:12, 12:25, 13:5, 15:37-40, Col 4:10` → **새로 보기** (한 패널에 모두 쌓임). `롬 11장`, `Acts 13-15` 같은 표기도 됩니다. **Enter** 만 누르면 화면을 안 바꾸고 팝업으로 먼저 미리봅니다.
2. **＋추가** 로 고른 구절을 **선택된 패널**(클릭해서 선택, 파란 테두리)에 쌓고, **＋새 패널** 로 빈 패널을 만든 뒤 블록의 **◀▶** 로 옮기면 좌→우로 나란히 비교할 수 있어요 (패널 최대 4개).
3. 각 구절 블록 버튼: **◀▶** 옆 패널로 이동 · **↦** 특정 패널 번호로 · **↑↓** 패널 안 순서 · **✂** 절 범위를 위/아래로 분할 · **📋** 본문 복사 · **★** 북마크 · **✕** 제거. 패널 상단 **🗑** 으로 패널 통째 삭제, **번갈아보기** 체크로 절 단위 묶음 보기 (패널별로 따로). 패널을 클릭해 선택하면 역본 체크박스·순서가 그 패널에만 적용됩니다.
...
- **도구 → 성경 다운로드…**: 번역본 6개 + 책 66권을 자유롭게 체크해서 부분 다운로드. *전체 / 구약 / 신약 / 모두 해제* 빠른 선택 버튼과 실시간 요약 (예: `27권 × 번역 1개 = 260회 호출 / 약 3분`). 이미 받은 장은 자동 skip 이라 도중 취소 → 다시 열기 → 이어받기 가능. 취소 버튼은 0.1초 안에 반응.
- **도구 → 검색…** (Ctrl+F): 캐시된 본문에서 한/영 부분 일치 검색. 번역본 필터 + 결과 미리보기. 더블클릭 시 선택된 패널에 추가(다이얼로그는 열린 채). *다운로드한 본문만 검색되니 도구 → 성경 다운로드… 를 먼저 돌려두세요.*
...
- **북마크 → 북마크 보기…**: 절 블록의 ★ 로 저장한 구절 목록. 더블클릭하면 선택된 패널에 추가, 삭제도 가능.
...
| 새번역 (SAENEW) | 대한성서공회 (bskorea.or.kr) | 대한성서공회 |
...
- **새 번역본 fetcher** — 쉬운성경 같은 한국어 번역의 무료 공개 소스를 알면 `fetchers.py` 에 추가 (개역개정·새번역·우리말성경·현대인의 성경·NIV·ESV 는 이미 연동)
...
ref_parser.py      자유 텍스트 성구 참조 파서 ("Acts 12:12, Col 4:10", "Acts 13-15", "롬 11장")
